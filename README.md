...
### “모델이 왜 틀리는가”를 실험으로 규명하고, 그 답을 파이프라인으로 옮깁니다.

생명과학 도메인의 문제를 머신러닝으로 다룹니다. 통계적 추론과 실험 설계가 무기입니다.
...
- 🧬 **서울대 항체·면역학 연구실 (BK21) 학부연구생** — AlphaFold3 계열 구조 예측 모델이 항체–항원 도킹에서 왜 특정 에피토프로 쏠리는지, 그 편향을 MSA로 진단하고 되돌리는 연구를 합니다.
- 💊 **데이터사이언스 학회 투빅스(ToBig's) 25·26기** — 25기 컨퍼런스에서 CV+LLM 복약지도 시스템 **PILLAR**를 PM으로 이끌었고, 26기 정규세션에서 *통계적 추론과 회귀분석* 강의를 맡았습니다.
- 📊 **엔드투엔드로 만듭니다** — 데이터 수집·라벨링부터 학습, 융합·랭킹, DB 검색, LLM 리포트 생성, 웹 데모 배포까지 한 줄기로 끌고 갑니다.
- 🔍 **결과보다 반증을 먼저 봅니다** — budget-matched control, 순열검정, 시드 재현성 게이트를 붙여 “정말 그 효과가 맞는지”부터 확인합니다.
...
| | 내용 |
|---|---|
| 🧬 **연구** | 항원 MSA 구성이 결합자리 선택을 좌우한다는 결과를 RBD에서 확인, **HA·Env 타깃으로 확장 검정 중** |
| 📐 **관심 문제** | 구조 예측 모델의 **학습 편향을 정량 진단하고 되돌리는 방법**, 그리고 생성(generation)과 선택(selection)을 한 쌍으로 푸는 설계 |
| 🎓 **교육** | 투빅스 26기 정규세션 — 통계적 추론·회귀분석 강의 및 과제 운영 |
| ✅ **완료** | PILLAR `v1.0` 릴리스 (알약 인식 → DUR 검색 → 복약지도서 생성 E2E) |
